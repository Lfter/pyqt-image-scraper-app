...
class LaunchAppTests(unittest.TestCase):
    ...
    def test_run_app_bat_points_to_windows_venv_python(self):
        launcher_text = (PROJECT_ROOT / "run_app.bat").read_text(encoding="utf-8")
        self.assertIn(r".venv\Scripts\python.exe", launcher_text)

    def test_run_app_bat_invokes_shared_launch_script(self):
        launcher_text = (PROJECT_ROOT / "run_app.bat").read_text(encoding="utf-8")
        self.assertIn(r"scripts\launch_app.py", launcher_text)
        self.assertIn("Please create .venv", launcher_text)
